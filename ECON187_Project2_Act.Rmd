...
```{r}
# Empty List to Store Performance Scores for Each Predictor
variable_list <- list()

# Variables to process
variables <- c("Stimulants", "Pulses", "Vegetable.Oils", "Miscellaneous", "Undernourished",
               "Milk...Excluding.Butter", "Fish..Seafood", "Vegetal.Products", "Animal.Products",
               "Animal.fats", "Eggs", "Obesity")
...
for (variable in variables) {
  ...
  for (i in 1:length(folds)) {
    ...
    for (j in 4:15) {
      formula <- paste("Deaths ~ poly(", variable, ", degree = ", j, ", raw = TRUE)", sep = "")
  ...
  variable_list[[variable]] <- c(cv_error_df[which.min(cv_error_df$Means), 1], min(cv_error_df$Means))
}
...
for (i in 1:length(folds)){
  ...
  nutrition_test <- nutrition_cln[test_index,]
  for (j in 4:15){
    fit <- lm(Deaths ~ poly(Animal.Products, degree = j, raw = TRUE), data = nutrition_cln, 
              subset = train_index) # Fit Piecewise Polynomial with specified degree
    prediction <- predict(fit, newdata = nutrition_test) # Predict on Test Set
    mse_score <- mean((nutrition_test$Deaths - prediction)^2) # MSE Calculation
    emptyvector[j-3] <- round(mse_score, 6)
  }
...
cv_error_df$Means <- round(rowMeans(cv_error_df[, 2:6]), 6)
print(cv_error_df)
print(paste0("Minimum Degree of Freedom for Animal.Products Piecewise Polynomial: ",
...
# Read and Clean Part II Data
```{r}
# Read Data into Environment
credit <- read.csv("credit_classification.csv", sep = ",",
                      header = TRUE)
head(credit)
...
# Omit NA vals
credit_cln <- na.omit(credit)

# Check again for NA vals on cleaned data
has_na_2 <- any(is.na(credit_cln))
...
# 
# reg.tree.model <- tree(Credit_score ~ Monthly_Balance, data=credit_cln)
# plot(reg.tree.model)
# text(reg.tree.model, cex=.75)
```

# Classification Tree
```{r}
# class.tree.model <- tree(log(MedianHouseValue) ~ Longitude + Latitude, data=real.estate)
# plot(class.tree.model)
# text(class.tree.model, cex=.75)
```
...
y <- nutrition$Obesity
X <- nutrition$Undernourished
X <- as.numeric(as.character(X))
missing_rows <- is.na(X) | is.na(y) 
y <- y[!missing_rows]
X <- X[!missing_rows]

breakpoints <- seq(10, 50, by = 10)  # Set the breakpoints increment and range
pred_terms <- lapply(breakpoints, function(b) I((X - b) * (X >= b)))

pred7 <- predict(lm(y ~ X + Reduce("+", pred_terms), data = nutrition))

# Create a dataframe with the clean variables
pred_df <- data.frame(X = X, pred7 = pred7, y = y)

tri.age.plot <- ggplot(pred_df, aes(x = X, y = y)) +
  geom_point(alpha = 0.55, color = "black") +
  theme_minimal()

tri.age.plot +
  geom_line(data = pred_df, aes(y = pred7), size = 1, col = "blue")
