...
for (i in 1:5){
  ...
  for (j in Shrinkage){
    ...
    accuracy <- sum(gbm.predict == airbnb.c.test$Guest.Satisfaction.r)/length(airbnb.c.test)
...
for (i in 1:5){
  # Specify the Fold
  training.i <- folds[[1]]
  testing.i <- set.diff(1:length(airbnb.cln.c$Guest.Satisfaction.r), training.i)
  airbnb.c.test <- airbnb.cln.c[testing.i,]
  ...
  for (j in N.Trees){
    # Fit the Model
    gbm.model <- gbm(as.factor(Guest.Satisfaction.r) ~., data = airbnb.cln.c,
                     subset = training.i, distribution = "bernoulli", 
                     n.trees = j)
...
# Tune Function
# tune.gbm.r <- tune(gbm, as.factor(Guest.Satisfaction.r) ~., data = airbnb.cln.c,
#                    ranges = list(n.trees = c(100,200,500,1000,1500), 
#                                  shrinkage = c(0,0.1,0.2,0.5,1.0,2.0)))
```
