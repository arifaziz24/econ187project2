...
```{r}
library(gam)
gam1 <- gam(Deaths~ s(Eggs), data = nutrition)
...
gam1_preds <- numeric(length(nutrition$Deaths))

# CV loop
for (i in 1:length(folds)) {
  train_index <- folds[[i]]
  test_index <- setdiff(1:length(nutrition$Deaths), train_index)
  
  # Test set preds
  gam1_test_preds <- predict(gam1, newdata = nutrition[test_index, ])
