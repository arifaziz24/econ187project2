...
reg.tree <- tree(Price~., airbnb.cln.r)
summary(reg.tree)

plot(reg.tree)
text(reg.tree, pretty = 0, cex = 1)

# Cross validation to determine what pruning, if any, is necessary
cv.air.r <- cv.tree(reg.tree, K=5)
plot(cv.air.r$size, cv.air.r$dev, type = 'b')

```

Here we have fit the regression tree using the response variable Price and all predictors. The preliminary regression tree model and cross validation indicate that the best pruning metric is at "best = 5" (given the fully grown tree has 5 terminal nodes, this outcome suggests that no pruning is necessary). We will now run the cross validation manually to allow us to use consistent folds across models. The manual cross validation will also allow us to calculate MSE and compare models using the metric. 
...
best.val <- c(2, 3, 4, 5)
reg.tree.all.folds.mse <- data.frame(best.val)

for (i in 1:5){
  # Specify the Fold
  training.i <- Fold[[1]]
  testing.i <- setdiff(1:length(airbnb.cln.c$Guest.Satisfaction.r), training.i)
  airbnb.r.test <- airbnb.cln.r[testing.i,]
  airbnb.r.train <- airbnb.cln.r[training.i,]
  
    # Empty the Vector Before Each Fold Run
  reg.tree.mse <- vector()
  
  for (j in 2:5){
    # Fit the Model
    reg.tree.cv <- tree(Price~., airbnb.r.train)
    reg.tree.prune <- prune.tree(reg.tree.cv, best = j) 
    
    # Predict the Model
    reg.prune.predict <- predict(reg.tree.prune, newdata = airbnb.r.test) 
    
    # Predict the Accuracy of the Model Using MSE
    mse_score <- mean((airbnb.r.test$Price-reg.prune.predict)^2)

    # Input MSE Scores Into Vector
    reg.tree.mse <- append(reg.tree.mse, mse_score)     
  
  }
   # Append our MSE Vector into Our Data Frame (Memory)
  reg.tree.all.folds.mse <- cbind(reg.tree.all.folds.mse, reg.tree.mse)
}
reg.tree.all.folds.mse
rowMeans(reg.tree.all.folds.mse[,-1])
```

The manual cross validation once again confirms that the "best" value to use to prune the regression tree that produces the lowest MSE is 5. Because the fully grown tree has 5 terminal nodes, this implies that no pruning is necessary. This gives us an MSE of 41588.38. 

# Classification Tree:
```{r}

#Fit Classification Tree
class.tree <- tree(as.factor(Guest.Satisfaction.r)~., airbnb.cln.c)
summary(class.tree)

plot(class.tree)
text(class.tree, pretty=0, cex=1)

# Cross validation to determine what pruning, if any, is necessary
cv.air.c <- cv.tree(class.tree, K=5)
plot(cv.air.c$size, cv.air.c$dev, type = 'b')

```

We have now fit the Classification tree model using Guest.Satisfaction.r as the response and all predictors available. The cross validation function has suggested that 4 is the best pruning value, and given the fully grown tree has only 4 terminal nodes, this output suggests that no pruning is necessary. Once again we will run the cross validation manually to use consistent folds across models and calculate the average MSE across folds while varying the number of terminal nodes through pruning.

```{r}
# Classification cross validation:

class.tree.mse <- vector()

best.val.c <- c(2, 3, 4)
class.tree.all.folds.acc <- data.frame(best.val.c)

for (i in 1:5){
  # Specify the Fold
  training.i <- Fold[[1]]
  testing.i <- setdiff(1:length(airbnb.cln.c$Guest.Satisfaction.r), training.i)
  airbnb.c.test <- airbnb.cln.c[testing.i,]
  airbnb.c.train <- airbnb.cln.c[training.i,]
   
    # Empty the Vector Before Each Fold Run
  class.tree.err <- vector()
  
  for (j in 2:4){
    # Fit the Model
    class.tree.cv <- tree(as.factor(Guest.Satisfaction.r)~., airbnb.c.train)
    class.tree.prune <- prune.tree(class.tree.cv, best = j) 
    
    # Predict the Model
    prune.predict <- predict(class.tree.prune, newdata = airbnb.c.test, type = "class") 
    
    # Predict the Accuracy of the Model
    accuracy <- sum(prune.predict == airbnb.c.test$Guest.Satisfaction.r)/nrow(airbnb.c.test)
    
    # Error Score & Input into Vector (1- Accuracy)
    error <- 1-accuracy
    class.tree.err <- append(class.tree.err, error)
  
  }
  
   # Append our MSE Vector into Our Data Frame (Memory)
  class.tree.all.folds.acc <- cbind(class.tree.all.folds.acc, class.tree.err)
}
class.tree.all.folds.acc

```

The accuracy of the cross validation is suggesting uniform performance across all pruning values (0.2417596). Because of this and the previous CV suggestion of 4 being the best pruning value, we will not prune the tree.
...
# Create a list to store the model results for each fold
model_results <- list()

# Perform cross-validation
for (fold in 1:length(Fold)) {
  # Split data into training and testing sets based on the fold
  train_data <- airbnb.cln.r[Fold[[fold]], ]
  test_data <- airbnb.cln.r[-Fold[[fold]], ]
  
  # Create a random forest model
  rf_model <- randomForest(Price ~ ., data = train_data)
  
  # Store the model results
  model_results[[fold]] <- list(model = rf_model, test_data = test_data)
}

# Access model results for a specific fold (e.g., fold 1)
fold1_model <- model_results[[1]]$model
fold1_test_data <- model_results[[1]]$test_data

# Make predictions using the random forest model on the test data of fold 1
predictions <- predict(fold1_model, newdata = fold1_test_data)

# Evaluate the model performance (e.g., using Mean Squared Error)
mse <- mean((predictions - fold1_test_data$Price)^2)

mse

```

Running random forests on our AirBnB data returns a MSE of 18055.58 - using five fold cross validation. Compared to our basic regression tree (returning a MSE of 41588.38), our random forest algorithm dramatically outperforms it. This is probably due to the functionality of random forest algorithms, where we run multiple trees. 
...
for (fold in 1:length(Fold)) {
  # Split data into training and testing sets based on the fold
  train_data <- airbnb.cln.c[Fold[[fold]], ]
  test_data <- airbnb.cln.c[-Fold[[fold]], ]
  
  # Create a random forest model
  rf_model <- randomForest(Guest.Satisfaction.r ~ ., data = train_data, importance = TRUE)
...
predictions <- predict(fold1_model, newdata = fold1_test_data, type = "class")

# Accuracy Score
class.pred <- ifelse(predictions > 0.5, 1,0)
accuracy.c <- sum(class.pred == fold1_test_data$Guest.Satisfaction.r)/length(fold1_test_data$Guest.Satisfaction.r)
...
# mse <- mean((predictions - fold1_test_data$Guest.Satisfaction.r)^2)
# mae <- mean(abs(predictions - fold1_test_data$Guest.Satisfaction.r))
# rsquared <- 1 - sum((predictions - fold1_test_data$Guest.Satisfaction.r)^2) / sum((fold1_test_data$Guest.Satisfaction.r - mean(fold1_test_data$Guest.Satisfaction.r))^2)
# 
# print(mse)
# print(mae)
# print(rsquared)
...
After running random forests on our classification problem, we obtained a classification error score of 15.52% which is significantly lower than our basic classification tree. Random forests are more robust in that they test more tree structures. Thus, we expected a better classification performance. 

# Boosting: Classification Problem

For our boosting models, the main parameter we'd have to tune is the learning or shrinkage parameter. This parameter determines the learning rate of our boosting model. The smaller the value of our shrinkage parameter, gamma, the slower the learning process. Along with gamma, another parameter we could vary is the number of trees (or iterations) within our model. The more trees we use, the better our fit will be. However, this can lead to overfitting. There are a variety of methods to optimize for these parameters. The two main ways we'll be doing this is using **five fold cross-validation** (see the folds we made above) and the **train function** in R. The latter approach is more computationally efficient and will allow us to perform a grid search of the boosting model's parameters. 

### Parameter Search for Classification (Train Function)
```{r, echo = FALSE}
# For Loop: Five Fold CV to Determine Shrinkage
set.seed(1209745)
library(caret)
library(gbm)

# Train Function with Grid of Parameters
control <- trainControl(method = "cv", number = 5) # Set Five Fold Cross Validation 

# Create Parameter Grid
parameters <- expand.grid(n.trees = c(100,200,500,1000,1500),
                          interaction.depth = c(5,10,15,20,50,100),
                          shrinkage = c(0,0.1,0.2,0.5,1.0,2.0),
                          n.minobsinnode = c(5,10,50,100,200))

# Fit A Boosting (GBM) Model 
features.c <- airbnb.cln.c[, !colnames(airbnb.cln.c) %in% "Guest.Satisfaction.r"]

# Train Function 
# trained.gbm.fit.c <- train(x = features.c, y = as.factor(airbnb.cln.c$Guest.Satisfaction.r), 
#                            method = "gbm", trControl = control, 
#                            tuneGrid = parameters)
```

We've pasted the results from the train function below. 

```{r}
# Best Tuning Parameters for Classification Problem
...
### Shrinkage Parameter Search for Classification Problem (5 Fold Cross-Validation)
```{r}
# Five Fold Cross Validation: For Shrinkage Parameter

# Set Up Data Frame and Vectors
Shrinkage <- c(0,0.1,0.2,0.5,1.0,2.0)
gbm.df.c.shrink <- data.frame(Shrinkage)
...
for (i in 1:5){
  ...
  # Empty the Vector Before Each Fold Run
  gbm.err.c.vec <- vector()
  
  for (j in Shrinkage){
    # Fit the Model
    gbm.model <- gbm(Guest.Satisfaction.r ~., data = airbnb.c.train, 
                     distribution = "bernoulli", 
                     shrinkage = j) 
    
    # Predict the Binary Classifier in the Testing Set
    gbm.predict <- predict(gbm.model, newdata = airbnb.c.test, type = "response") 
    
    # Convert to Classification
    gbm.predict.c <- ifelse(gbm.predict > 0.5, 1,0)
    
    # Predict the Accuracy of the Model
    accuracy <- sum(gbm.predict.c == airbnb.c.test$Guest.Satisfaction.r)/nrow(airbnb.c.test)
    
    # Error Score & Input into Vector (1- Accuracy)
    error <- 1-accuracy
    gbm.err.c.vec <- append(gbm.err.c.vec, error)
  }
  
  # Append our Error Vector into Our Data Frame (Memory)
  gbm.df.c.shrink <- cbind(gbm.df.c.shrink, gbm.err.c.vec)
  
  # When the For Loop Runs Again, The Vector Will Be Emptied, Data Frame Maintains
}

# Data Frame Clean
gbm.df.c.shrink$Means <- rowMeans(gbm.df.c.shrink[,2:6]) 
colnames(gbm.df.c.shrink) <- c("Shrinkage","Fold 1","Fold 2", "Fold 3", "Fold 4",
                               "Fold 5", "CV Error Means")
print(gbm.df.c.shrink)
```

From running five fold cross validation to test a variety of shrinkage parameters, the most optimal one, which returned the lowest CV error score, was a shrinkage parameter value of 0.5. The shrinkage parameter value of 0.2 was a very close second - which matches our findings from the train function we ran above. Moving forward, we'd recommend using the train function to identify the most optimal parameters. Looking at each parameter individually just helps to confirm our findings through the train function (a form of a sanity check). We'll still run cross validation to see how the model performs across five folds.  

### Number of Trees Parameter Search (5 Fold Cross-Validation)
```{r}
# Five Fold Cross Validation: For Number of Trees
N.Trees <- c(100,200,500,1000,1500)
gbm.df.c.ntree <- data.frame(N.Trees)
...
for (i in 1:5){
  ...
  for (j in N.Trees){
    ...
    gbm.predict.c <- ifelse(gbm.predict.c > 0.5, 1,0)
  ...
  # Append our Error Vector into Our Data Frame (Memory)
  gbm.df.c.ntree <- cbind(gbm.df.c.ntree, gbm.err.c.vec)
  
  # When the For Loop Runs Again, The Vector Will Be Emptied, Data Frame Maintains
}

# Data Frame Clean
gbm.df.c.ntree$Means <- rowMeans(gbm.df.c.ntree[,2:6]) 
colnames(gbm.df.c.ntree) <- c("Trees","Fold 1","Fold 2", "Fold 3", "Fold 4",
                               "Fold 5", "CV Error Means")
print(gbm.df.c.ntree)
```

After running cross validation on the number of trees, we find that varying them from the levels we specified does not really make a difference on how well the boosting algorithm performs in testing errors. This shows that the boosting algorithm converges to a certain prediction level, with respect to the number of trees. Other parameters, such as shrinkage, may be more optimal in improving accuracy. The average error score for our varying levels of trees is quite high (greater than 75%). 

### Optimal Boosting Algorithm Model for Classification Problem
```{r}
# Cross Validation to Check Accuracy of GBM
gbm.opt.err.vector <- vector() # Vector to Input Errors
...
for (i in 1:5){
  ...
  # Fit Model According to "Best Tune" Feature of Trained Function
  gbm.model <- gbm(Guest.Satisfaction.r ~., data = airbnb.c.train, 
                     distribution = "bernoulli", 
                     shrinkage = 0.2, n.trees = 1500, 
                   interaction.depth = 20, n.minobsinnode = 5) 
  ...
  error.c <- 1-accuracy
  gbm.opt.err.vector[i] <- error.c
}

# Print Vector of Errors & Show Mean of CV Error
print(gbm.opt.err.vector)
mean(gbm.opt.err.vector)
```

Using our most optimal boosting model for the classification problem, our mean prediction error, using cross validation, is 16.5%. The optimal model uses parameters identified using our hyper-parameter grid search through the train function. This error metric is significantly lower than other error metrics from the different boosting models we experimented with (cross-validation algorithms searching for an optimal parameter). We'll see how this fares with other tree algorithms we have used. 
...
# Fit A Boosting (GBM) Model 
features.r <- airbnb.cln.r[, !colnames(airbnb.cln.r) %in% "Price"]

# # Trained Function
# trained.gbm.fit.r <- train(x = features.r, y = airbnb.cln.r$Price, 
#                            method = "gbm", trControl = control, 
#                            tuneGrid = parameters.r)
```

We'll paste the output (optimized boosting model parameters) from the train function below. The markdown version of our project will not show the output of the train function because it takes too long to run (~ 4 hours). An optimal boosting model would create 1500 separate trees (each with an interaction depth of 20), along with a shrinkage parameter of 0.1. Each split would require at least 5 observations, according to our train function. 
...
### Shrinkage Parameter Search for Regression Problem
```{r}
# Five Fold Cross Validation: For Shrinkage Parameter

# Set Up Data Frame and Vectors
Shrinkage <- c(0,0.1,0.2,0.5,1.0,2.0)
gbm.df.r.shrink <- data.frame(Shrinkage)

for (i in 1:5){
  # Specify the Fold
  training.i <- Fold[[1]]
  testing.i <- setdiff(1:length(airbnb.cln.r$Price), training.i)
  airbnb.r.test <- airbnb.cln.r[testing.i,]
  airbnb.r.train <- airbnb.cln.r[training.i,]
  
  # Empty the Vector Before Each Fold Run
  gbm.err.r.vec <- vector()
  
  for (j in Shrinkage){
    # Fit the Model
    gbm.model <- gbm(Price ~., data = airbnb.r.train, 
                     distribution = "gaussian", 
                     shrinkage = j) 
    
    # Predict the Model
    gbm.predict <- predict(gbm.model, newdata = airbnb.r.test) 
    
    # Predict the Accuracy of the Model Using MSE
    mse_score <- mean((airbnb.r.test$Price-gbm.predict)^2)

    # Input MSE Scores Into Vector
    gbm.err.r.vec <- append(gbm.err.r.vec, mse_score)     

  }
  
  # Append our MSE Vector into Our Data Frame (Memory)
  gbm.df.r.shrink <- cbind(gbm.df.r.shrink, gbm.err.r.vec)
  
  # When the For Loop Runs Again, The Vector Will Be Emptied, Data Frame Maintains
}

# Data Frame Clean
gbm.df.r.shrink$Means <- rowMeans(gbm.df.r.shrink[,2:6]) 
colnames(gbm.df.r.shrink) <- c("Shrinkage","Fold 1","Fold 2", "Fold 3", "Fold 4",
                               "Fold 5", "CV Error Means")
print(gbm.df.r.shrink)
```

A shrinkage parameter of 0.2 seems to minimize average MSE from five fold cross validation. A shrinkage parameter of 0.1, which the train function confirmed, does just as well compared to other parameter values. 

### Number of Trees Search for Regression Problem
```{r}
# Five Fold Cross Validation: For Number of Trees
N.Trees <- c(100,200,500,1000,1500)
gbm.df.r.ntree <- data.frame(N.Trees)
...
# Empty the Vector Before Each Fold Run
  ...
  # Append our MSE Vector into Our Data Frame (Memory)
  gbm.df.r.ntree <- cbind(gbm.df.r.ntree, gbm.err.r.vec)
  
  # When the For Loop Runs Again, The Vector Will Be Emptied, Data Frame Maintains
}

# Data Frame Clean
gbm.df.r.ntree$Means <- rowMeans(gbm.df.r.ntree[,2:6]) 
colnames(gbm.df.r.ntree) <- c("Trees", "Fold 1","Fold 2", "Fold 3", "Fold 4",
                               "Fold 5", "CV Error Means")
print(gbm.df.r.ntree)
```

The number of trees that minimizes average MSE scores across all five folds is 500. Unlike with the shrinkage parameter, the average MSE scores are quite comparable. We saw this with the number of trees in the classification problem (to an extreme where the errors all match). We can see that 1500 trees, which our train function recommends, does perform quite well.

Moving forward as we did with our classification problem, we'll use the hyperparameters identified in our train function to fit our optimal boosting algorithm. We'll use five fold cross validation to compare our models to others. 

### Optimal Boosting Algorithm for Regression Problem
```{r}
# Cross Validation to Check Accuracy of GBM Model
gbm.opt.err.vector <- vector() 
...
for (i in 1:5){
  ...
  gbm.model <- gbm(Price ~., data = airbnb.r.train, 
                   ...
                   n.trees = 1500, interaction.depth = 20, n.minobsinnode = 5)
  
  # Predict the Model on New Data
  gbm.predict <- predict(gbm.model, newdata = airbnb.r.test, type = "response")
  
  # Calculate MSE Scores
  gbm.opt.err.vector[i] <- mean((airbnb.r.test$Price-gbm.predict)^2)
  }
...
Using our optimal boosting algorithm (with parameters defined by the train function), our algorithm returns an average MSE across five folds of 25325.31. This is much lower than the average MSE scores we saw in our other cross validation methods, where we tried to optimize for shrinkage and number of trees. When comparing our boosting algorithm to other techniques, we'll use this boosting algorithm as a benchmark. 

# Conclusion

After running different models on our AirBnB data, we find that our random forests models outperform in both classification and regression problems. Our random forest models returned lower MSE scores and classification error scores. Our second highest performing algorithm were gradient boosting models, with comparable error metrics in classification. Random forests and gradient boosting models are more robust in that they run multiple tree structures within our data cube. We'd expect one of these advanced algorithms to outperform simple trees. In conducting this comparative analysis, we mainly used five fold cross validation and a variety of train functions through a selection of R Packages. Below are the final rounded results.

Regression Problem Error: Basic Regression Tree (41588.38), Random Forest (18055.58), Boosting (25325.31)

Classification Problem Error: Basic Classification Tree (24.17%), Random Forest (15.52%), Boosting (16.50%). 
