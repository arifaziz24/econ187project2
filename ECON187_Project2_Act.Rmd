...
According to our Boruta plot, thirteen out of the twenty three predictors we shortlisted are significant in explaining and predicting COVID-19 deaths. Moving forward, we'll be using these 10 predictors and applying specific transformations of them (splines, polynomials) to predict COVID-19 deaths.  

# Relationship Between Response Variable & Chosen Predictors
```{r}
# Remove Variables Not Approved by Boruta Algorithm:
nutrition_cln <- nutrition %>% select(-c("Fruits...Excluding.Wine","Spices",
                                         "Sugar...Sweeteners","Vegetables",
                                         "Offals","Treenuts","Meat",
                                         "Starchy.Roots"))

# Plot For Function: Break Up Variables in 8 Predictor Chunks for Plots
for (variable in names(nutrition_cln)){
  plot(nutrition_cln[[variable]], nutrition_cln$Deaths, main = paste0(variable," Against COVID-19 Deaths"), ylab = "Population Proportion of Death", xlab = variable)
} 
